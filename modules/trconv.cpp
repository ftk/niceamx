...
static const char en_ru_tralphabet[] = "фисвуапршолдьтщзйкыегмцчня"; // abcdefghijkl....
static const char ru_en_tralphabet[] = "f,dult;pbqrkvyjghcnea[wxio]sm\'.z"; // пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ...
...
#define isupper_cyr(ch) (((ch)>='А')&&((ch)<='Я'))
#define islower_cyr(ch) (((ch)>='а')&&((ch)<='я'))

#define toupper_cyr(ch) (islower_cyr(ch)?((ch)-'а'+'А'):(ch))
#define tolower_cyr(ch) (isupper_cyr(ch)?((ch)-'А'+'а'):(ch))
      ...
      else // non-alpha
      {
        for(unsigned int j=0; j<sizeof(en_ru_transform); j++)
        {
          ...
          else if(text[i] == en_ru_transform[j][1])
          {
            ...
            if(tr == 1)
                text[i] = tmp != 'ё' ? toupper_cyr(tmp) : 'Ё';
            else if(tr == 3)
